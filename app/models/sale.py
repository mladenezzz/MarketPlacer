...
class Sale(db.Model):
    """Модель для хранения продаж маркетплейсов"""
    __tablename__ = 'sales'
    ...
    sale_type = db.Column(db.String(20), nullable=True)  # 'FBS', 'FBO' для Ozon, None для Wildberries
    
    # Идентификаторы продажи
    ...
    # Финансовые данные
    price = db.Column(db.Float, nullable=False, default=0.0)  # Цена продажи
    ...
    # Связь с токеном
    token = db.relationship('Token', backref=db.backref('sales', lazy=True, cascade='all, delete-orphan'))
    
    # Уникальный индекс для предотвращения дубликатов
    __table_args__ = (db.UniqueConstraint('token_id', 'sale_id', 'marketplace', name='uq_sale_token_marketplace'),)
    ...
    def __repr__(self):
        return f'<Sale {self.sale_id} for token {self.token_id}>'
