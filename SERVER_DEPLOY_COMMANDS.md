...
- ✅ **ИСПРАВЛЕНИЕ**: Datacollector собирает все заказы Ozon со вчерашнего дня для учета задержки API
- ❌ НЕ запускайте migrate_ozon_sales.py на сервере
- ❌ НЕ запускайте reset_ozon_sync.py на сервере

## Последние изменения:

### Commit dc902d6 - fix: collect all Ozon orders from yesterday to handle API delay
**Проблема:**
- API Ozon имеет значительную задержку в отображении заказов (несколько часов)
- Перекрытие в 1-3 часа не всегда достаточно
- Невозможно предсказать точное время задержки API

**Решение:**
- Теперь datacollector всегда запрашивает все заказы со вчерашнего дня (00:00)
- Гарантирует получение всех заказов независимо от задержки API
- Проверка дубликатов (posting_number + sku) предотвращает дублирование
- Каждый запуск обрабатывает ~100-200 заказов вместо выборочного диапазона
- Более надежный подход без риска пропуска заказов

### Commit f425777 - fix: increase Ozon orders overlap from 1 to 3 hours (устарело)
**Проблема:**
